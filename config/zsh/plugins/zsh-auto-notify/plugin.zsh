...
function _auto_notify_message() {
    ...
    if [[ "$platform" == "Linux" ]]; then
      ...
      terminal-notifier \
        -title "$command" \
        -message "$body" \
        -appIcon /Applications/WezTerm.app/Contents/Resources/terminal.icns \
        -open "file:///Applications/WezTerm.app"
            else
...
function humanize_seconds() {
    ...
    if (( total_seconds < 0 )); then
        echo "Invalid input: seconds must be non-negative."
        return 1
    fi
...
function _auto_notify_send() {
    ...
    if [[ "$(_is_auto_notify_ignored "$AUTO_COMMAND_FULL")" == "no" ]]; then
        ...
        if [[ $elapsed -gt $AUTO_NOTIFY_THRESHOLD ]]; then
            local duration=$(humanize_seconds $elapsed)
            _auto_notify_message "$AUTO_COMMAND" "$duration" "$exit_code"
        fi
    fi
